...
|---------------------------|------------------------------|---------|------------------------------------------------------------------|
| **data** <RequiredLabel/> | string \| Buffer             |         | Buffer or string to hash                                         |
| **options**               | [GenericOptions](#options-4) | `{}`    | Optional configuration object for input data encoding and output |
...
async exampleFastHashSHA1(): string {
  const sha1Hash = this.cryptographyService.createInsecureFastHash(
